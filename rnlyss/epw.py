...
def epw_years(epw):
    ...
    # Load year and month
    df = pd.read_csv(epw, header=None, usecols=[0, 1], dtype=int,
                     skiprows=8, names=['Y', 'M'])
...
def epw_col(epw, col=6):
    ...
    df = pd.read_csv(epw, header=None, usecols=[0, 1, 2, 3, col],
                     skiprows=8, names=['Y', 'M', 'D', 'H', 'X'])
    ...
    df['Date'] = join_date(y=df['Y'], m=df['M'], d=df['D'],
                           hh=df['H']-1, mm=30)
...
def write_solar_csv(path, dsets, years, lat=0, lon=0, hgt=0, tz=0, **kwargs):
    ...
    df = pd.concat([
        dset.solar_split(lat, lon, years=data_years) for dset in dsets
    ])
...
def write_epw(path, dsets, years, lat=0, lon=0, hgt=0, tz=0, **kwargs):
    ...
    # and wind direction
    df = pd.concat([
        dset.to_hof(lat, lon, hgt=hgt, years=data_years) for dset in dsets
    ])
    ...
    pr = pd.concat([
        dset("pr", lat, lon, hgt=hgt, years=data_years) for dset in dsets
    ])
    if pr is not None:
        # Convert to mm
    ...
    for i in range(12):
        ...
        ts.append(pd.Series(index=pd.date_range(
            start=t0-dt,
            periods=24*n_days+2*w,
            freq="h"
    ...
    df['It'], df['Ib'], df['Id'], df['Lz'] = perez(df['Eb'], df['Ed'],
                                                   df['E0'], df['E0h'],
                                                   df['DP'])
    ...
    for name in snaps:
        ...
        sources.append(name)
        comments.append(
            "%s %r" % (name, sorted(list(set(snaps[name]['years']))))
        )
    ...
    T_gnd = calc_ground_temperatures(
        T_mean, T_min, T_max, coldest_month,
        depth=depths, leap_year=is_leap(years[1]),
    ...
    with open(path, "w") as epw:
        ...
        epw.write(
            "COMMENTS 1, Compiled from %s reanalysis data; %d%% land\n" % (
                " and ".join(comments),
                int(np.rint(100*land))
            )
        )
        epw.write("COMMENTS 2, Ground temps produced with a standard soil"
...
def main():
    ...
    # from selected years
    years = [2011, 2008, 2005, 2005, 2012, 1990,
             2003, 1985, 2017, 1998, 1998, 1980]
    meta = {'city': 'Atlanta', 'state': 'GA', 'country': 'USA'}
    loc = {'lat': 33.640, 'lon': -84.430, 'hgt': 313}
    write_epw('Atlanta.epw', dsets=['CFSR', 'CFSV2'], years=years,
              **meta, **loc)
